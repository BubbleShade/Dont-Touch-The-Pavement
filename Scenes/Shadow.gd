...
@export var ShadowProgress = 0.5
@export var shadowScale = 5
...
func _process(delta):
	...
	var modSP = abs(fmod(abs(ShadowProgress), 2.0) -1)
	...
	# when modSP = 1, the x compenent is 0
